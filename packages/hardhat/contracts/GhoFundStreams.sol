...
contract GhoFundStreams is Ownable {
	...
	function addBuilderStream(address payable _builder, uint256 _GHOcap) public onlyOwner {
		streamedBuilders[_builder] = BuilderStreamInfo(_GHOcap, block.timestamp - frequency);
		emit AddBuilder(_builder, _GHOcap);
	}

	function addBuilderStreamBatch(address[] memory _builders, uint256[] memory _GHOcaps) public onlyOwner {
